...
export default function Ninja() {
  ...
  function drawBackground() {
    // Draw sky
    var gradient = canvasRef.current.getContext("2d").createLinearGradient(0, 0, 0, window.innerHeight);
    gradient.addColorStop(0, "#BBD691");
    gradient.addColorStop(1, "#FEF1E1");
    canvasRef.current.getContext("2d").fillStyle = gradient;
    canvasRef.current.getContext("2d").fillRect(0, 0, window.innerWidth, window.innerHeight);
  ...
  function drawHill(baseHeight, amplitude, stretch, color) {
    ...
    canvasRef.current.getContext("2d").lineTo(window.innerWidth, window.innerHeight);
  ...
  return (
      <div className="w-screen h-full bg-[#02121d]">
          <div className="container mx-auto cursor-pointer flex flex-col justify-center items-center h-full font-body">
              <div ref={scoreRef} className="absolute top-[10rem] right-1/2 -translate-x-1/2 text-2xl font-bold"></div>
              <canvas ref={canvasRef} width="375" height="375" />
              <div ref={introRef} className={`w-[200px] h-[150px] absolute font-semibold text-lg text-center`}>
                  Hold down the mouse to stretch out a stick
              </div>
              <div ref={perfectRef} className={`absolute opacity-0`}>DOUBLE SCORE</div>
              <button onClick={restartGame} ref={restartRef} className={`absolute border-none w-[120px] h-[120px] hidden rounded-full text-white bg-red-400 font-semibold text-xl cursor-pointer`}>RESTART</button>
          </div>
      </div>
  )
