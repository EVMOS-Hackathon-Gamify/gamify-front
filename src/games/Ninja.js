...
export default function Ninja() {
    ...
    function animate(timestamp) {
        ...
        switch (phase) {
          ...
          case "transitioning": {
            setSceneOffset(sceneOffset + (timestamp - lastTimestamp) / transitioningSpeed)
    ...
    return (
        <div ref={elemRef} className="w-screen h-full bg-[#02121d]">
            <div onKeyDown={keydownEvent} onMouseDown={mousedownEvent} onMouseUp={mouseupEvent} className="container mx-auto cursor-pointer flex flex-col justify-center items-center h-full font-body">
                <div id="score" className="absolute top-[30px] right-[30px] text-2xl font-bold">{score}</div>
                <canvas ref={canvasRef} width="375" height="375" />
                <div id="introduction" className={`w-[200px] h-[150px] absolute font-semibold text-lg text-center opacity-${introductionOpacity}`}>
                    Hold down the mouse to stretch out a stick
                </div>
                <div id="perfect" className={`absolute opacity-${perfectOpacity}`}>DOUBLE SCORE</div>
                <button id="restart" onClick={() => {
                    resetGame()
                    setRestartDisplay("hidden")
                }} className={`absolute border-none ${restartDisplay} w-[120px] h-[120px] rounded-full text-white bg-red-400 font-semibold text-xl cursor-pointer`}>RESTART</button>
            </div>
        </div>
    )
}
